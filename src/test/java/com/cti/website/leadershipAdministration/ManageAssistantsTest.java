...
/**
 * This test covers the Leadership Administration website  - Manage Assistants link
 * @author Preethi
 */

public class ManageAssistantsTest extends Common{
	...
	public void manageLeaders() {
		...
		//To click on Manage Assistants link
		click(hashMap.get(CommonConstants._MNG_ASSTS));
		Assert.assertTrue(verifyElementPresent(hashMap.get(CommonConstants._LEADERSHIP_ADMIN_TRIBE)));
		Assert.assertEquals(getText(hashMap.get(CommonConstants._LEADERSHIP_ADMIN_TRIBE)), "Launchpad Administration - Assistants");
		...
		Assert.assertTrue(verifyElementPresent(hashMap.get(CommonConstants._AGREED)));
		Assert.assertTrue(verifyElementPresent(hashMap.get(CommonConstants._ADD_ASST))); 
		
		//click(_ADD_ASST);
		//Clicking on Add Assistant link adds another Assistant to the program, hence it is not automated.
